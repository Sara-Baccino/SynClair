...
import { createDatasetFromArtifact } from "../../../api/client";
...
function ArtifactPromotionPanel({ jobId, artifact }: { jobId: string; artifact: DataFramePreviewDTO }) {
  ...
  // Detect discrete columns from the preview (few distinct values), so
  // the user can filter without a free-form expression editor.
  const discreteColumns = artifact.columns.filter((col) => {
    const distinctValues = new Set(artifact.preview.map((row) => row[col]));
    return distinctValues.size > 0 && distinctValues.size <= 10;
  });

  const [filterColumn, setFilterColumn] = useState<string>("");
  const [selectedValues, setSelectedValues] = useState<Set<string>>(new Set());

  const distinctValuesForFilterColumn = filterColumn
    ? Array.from(new Set(artifact.preview.map((row) => String(row[filterColumn] ?? ""))))
    : [];
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return createDatasetFromArtifact({
        source_job_id: jobId,
  ...
  return (
    <div className="mt-4 rounded border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3">
          ...
          {filterColumn && (
            <div className="mt-2 flex flex-wrap gap-3">
              {distinctValuesForFilterColumn.map((value) => (
      ...
      {mutation.isError && (
        <p className="mt-2 text-xs text-red-600">Failed to promote this artifact. Please try again.</p>
      )}
