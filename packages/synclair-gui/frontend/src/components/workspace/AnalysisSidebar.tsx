...
import { getStructureJobStatus } from "../../api/client";
...
export function AnalysisSidebar() {
  ...
  // Stato "git graph": ogni nodo done/current/pending, in base allo stato reale
  const nodeState = (path: string): "done" | "current" | "pending" => {
    ...
    if (["results", "artifacts", "export"].includes(path)) {
  ...
  return (
    <aside className="flex w-60 flex-shrink-0 flex-col border-r border-slate-200 bg-white p-4">
      <div className="mb-4 flex items-center justify-between">
        <Link
          to="/"
          className="text-sm font-bold tracking-tight bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 bg-clip-text text-transparent hover:opacity-80 transition"
        >
          SynClair
        </Link>
        <Link to="/workspace/modules" className="text-xs text-slate-500 hover:text-blue-600">
          ← Modules
        </Link>
      </div>

      <div className="mb-4 text-xs font-semibold uppercase tracking-wide text-slate-400">
        {moduleId} {datasetId ? "· dataset loaded" : ""}
      </div>
              ...
              {index < SECTIONS.length - 1 && (
                <div
                  className={`absolute left-[5px] top-4 h-full w-px ${
                    state === "done" ? "bg-blue-400" : "bg-slate-200"
                  }`}
                />
              )}
              <span
